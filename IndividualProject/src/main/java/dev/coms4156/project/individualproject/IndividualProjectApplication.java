...
public class IndividualProjectApplication implements CommandLineRunner {
	...
	public void resetDataFile() {
		...
		addDepartment(departmentMapping, "ECON", econCourseCodes, econCourses, "Michael Woodford", 2345);

		//data for ieor dept
		String[] ieorCourseCodes = {"2500", "3404", "3658", "4102", "4106", "4405", "4511", "4540"};
		List<Course> ieorCourses = getIeorCourses(locations, times);
		...
		List<Course> physCourses = getPhysCourses(locations, times);
	 ...
	 * 
	 * @param locations An array of {@code String} representing locations where the courses are held.
	 * @param times An array of {@code String} representing the times when the courses are scheduled.
	 ...
	 */
	private List<Course> getIeorCourses(String[] locations, String[] times) {
	 ...
	 */
	private List<Course> getPhysCourses(String[] locations, String[] times) {
	...
	/**
	 * Helper method to populate Hashmap of Courses in a department
	 * 
	 * @param courseCodes A {@Code String[]} of the course numbers
	 * @param courses A {@Code List<Course>} of the courses in a department  corresponding to course numbers
	 * @return A {@Code Hashmap} that is populated with course codes and their corresponding course
	 */
	private HashMap<String, Course> getCourseMap(String[] courseCodes, List<Course> courses) {
	...
	/**
	 * Add Department to the departmentMapping HashMap
	 * This method creates a {@Code Department} object using the provided details and adds it 
	 * to the {@Code HashMap} of departments. It also generates a mapping of course codes to 
	 * {@Code Course} objects and assigns this mapping to the new department.
	 * 
	 * @param departmentMapping A {@Code HashMap<String, Department>} where the key is the department 
	 *                          code and the value is the corresponding {@Code Department} object.
	 * @param departmentCode A {@Code String} representing the code identifying the department. 
	 * @param courseCodes A {@Code String[]} array of course codes corresponding to the courses 
	 *                    offered by the department. Each code will be used to map to its 
	 *                    respective {@Code Course} object.
	 * @param courses A {@Code List<Course>} of {@Code Course} objects that the department offers. 
	 * @param departmentChair A {@Code String} representing the name of the chairperson for the department.
	 * @param numberOfMajors An {@Code int} indicating the number of students majoring in the department.
	*/
	private void addDepartment(HashMap<String, Department> departmentMapping, 
								String departmentCode, 
								String[] courseCodes, 
								List<Course> courses, 
								String departmentChair, 
								int numberOfMajors) {
		HashMap<String, Course> courseMap = getCourseMap(courseCodes, courses);
		Department department = new Department(departmentCode, courseMap, departmentChair, numberOfMajors);
	 ...
	 * 
	 * @param courses A {@Code List<Course>} to which the new {@Code Course} will be added.
	 * @param instructor A {@Code String} representing the name of the instructor for the course.
	 * @param location A {@Code String} indicating the location where the course is held.
	 * @param time A {@Code String} specifying the time during which the course takes place.
	 * @param capacity An {@Code int} representing the maximum number of students that the course can accommodate.
	 * @param enrolled An {@Code int} indicating the number of students currently enrolled in the course.
	 */
	private void addCourse(List<Course> courses, String instructor, String location, String time, int capacity, int enrolled) {
